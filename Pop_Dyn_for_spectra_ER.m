...
epsi=10^-20;   %epsilon to be used in the final formula for the asd
lambdamax=3.5;
...
for s=1:lambda_size
    ...
    %a new population needed for any new value of lambda
     ...
     %degree sequence generation (once for all!)
     deg1=[];
     for i=1:kmax
    ...
    %generate degree sequence according to p(k)(once for all!)
        ...
        deg2=deg2(randperm(M*n));
         ...
         %degree sequence generation for cavity sweeps (once for all!)
        deg1=[];
        for i=1:kmax
